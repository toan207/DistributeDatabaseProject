...
as
begin
	if (exists (select AuthorID from [DESKTOP-84SCD87\SITE1].BookManagement.dbo.[AUTHOR_No_Genre] where AuthorID = @AuthorID))  
	begin
		print 'Khong the them vao AuthorID da co'
	end
	else
		begin
		insert into [DESKTOP-84SCD87\SITE1].BookManagement.dbo.AUTHOR_No_Genre(AuthorID, AuthorName, Sex, Nationality, DOB)
...
as
begin
	if (exists (select CustomerID from [DESKTOP-84SCD87\SITE1].BookManagement.dbo.CUSTOMERS_No_Address where CustomerID = @CustomerID))  
	begin
		print 'Khong the them vao CustomerID da co'
	end
	else
		begin
		insert into [DESKTOP-84SCD87\SITE1].BookManagement.dbo.CUSTOMERS_No_Address(CustomerID, CustomerName, City, FAX, Phone)
...
begin
	if (exists (select BookID from [DESKTOP-84SCD87\SITE1].BookManagement.dbo.BOOKSTOCK_HanoiMoreThan60000 where BookID = @BookID) 
	   ...
	   or not exists (select AuthorID from [DESKTOP-84SCD87\SITE1].BookManagement.dbo.AUTHOR_No_Genre where AuthorID = @AuthorID)
...
begin
	if (exists (select OrderID from [DESKTOP-84SCD87\SITE1].BookManagement.dbo.ORDERS_HanoiLater2021 where OrderID = @OrderID) 
	   ...
	   or not exists (select CustomerID from [DESKTOP-84SCD87\SITE1].BookManagement.dbo.CUSTOMERS_No_Address where CustomerID = @CustomerID)
	   or (@BranchID != 'B1' and @BranchID != 'B0'))
	begin
...
begin
	if (exists (select AuthorID from [DESKTOP-84SCD87\SITE1].BookManagement.dbo.[AUTHOR_No_Genre] where AuthorID = @AuthorID))
	begin
		delete from [DESKTOP-84SCD87\SITE1].BookManagement.dbo.AUTHOR_No_Genre
...
begin
	if (exists (select CustomerID from [DESKTOP-84SCD87\SITE1].BookManagement.dbo.[CUSTOMERS_No_Address] where CustomerID = @CustomerID))
	begin
		delete from [DESKTOP-84SCD87\SITE1].BookManagement.dbo.CUSTOMERS_No_Address
...
go
-- edit author proc
create proc edit_author @AuthorID varchar(2), @Genre nvarchar(60)
as
begin
	if (exists (select AuthorID from AUTHOR_Genre where AuthorID = @AuthorID))  
		update AUTHOR_Genre set Genre = @Genre where AuthorID = @AuthorID
	else
		print 'Khong co AuthorID nay'
end
go
-- edit customer proc
create proc edit_customer @CustomerID varchar(2), @Address nvarchar(30)
as
begin
	if (exists (select CustomerID from CUSTOMERS_Address where CustomerID = @CustomerID))
		update CUSTOMERS_Address set Address = @Address where CustomerID = @CustomerID
	else
